...
Then, on the next lines, a JSON block with your per-source citations:
```json
...
- Pick supporting quotes that state the sub-claim's actual property, not merely the same subject; code checks each quote against the claim, so an off-topic sentence earns nothing.
...
formula in the PARAMETERS only, e.g. fixed_point {{"x": "sqrt(a/b)", "y": "0"}}); give sim_criterion on the \
spectral abscissa alpha (max real eigenvalue of the Jacobian): "lt" a non-positive margin for a STABILITY claim \
(set robust_frac to 1 — any sampled point above the margin refutes), or "gt" a non-negative margin for an \
INSTABILITY-ONSET claim. Prefer a strict margin (a hyperbolic equilibrium). Give a param_sweep with enough \
points per axis. linear_stability uses NO t_span/dt/init/observable/max_steps/null_overrides — omit them.
