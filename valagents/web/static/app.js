...
function eventSummary(e) {
  switch (e.event) {
    ...
    case "check": return [e.verdict === "pass" ? "review" : "warn", `claim ${e.claim} · ${e.lens} · ${e.verdict}`];
    case "novelty": return ["search", `novelty · ${e.position || "--"} · ${clip(e.delta, 48)}`];
    case "completion": return ["gen", `completed candidate · ${e.status || "--"} · weakest ${clip(e.weakest_link, 32)}`];
    case "theory_bridge": return ["bridge", `theory bridge · ${clip(e.family, 36)}${e.nearest?.length ? ` · ${e.nearest.length} anchors` : ""}`];
    case "positioning": return ["bridge", `prior art · ${clip(e.closest_prior, 34)} · new: ${clip(e.what_is_new, 38)}`];
    case "known_limits": return ["review", `known limits · ${e.count || 0} checked · ${e.unclear || 0} unclear · ${e.failed || 0} failed`];
    case "convincing_case": return ["case", `convincing case · ${(e.skeptic_tests || []).length} skeptic tests`];
    case "predictions": return ["gen", `predictions · ${e.count || 0} total · ${e.measurable || 0} measurable`];
    case "redteam": return ["review", `red-team · ${e.attacks || 0} attacks · ${e.landed || 0} landed · ${(e.attempted || []).join(", ") || "none"}`];
    case "redteam_check": return [e.verdict === "fail" ? "warn" : "review", `red-team claim ${e.claim} · ${e.verdict}`];
    case "validation_plan": return ["done", `validation plan · ${e.cost || "--"} · ${clip(e.test, 52)}`];
    case "fanout": return ["review", `fanout claim ${e.claim} · ${e.lens} · ${e.verdict}`];
