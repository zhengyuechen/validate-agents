...
def _support_quote_valid(quote: str, source_text: str, claim_statement: str,
                         subject_tokens: set[str], min_tokens: int) -> bool:
    """§4/§5 supports-only gate = _quote_admissible AND the on-property floor. The distinctive set is derived
    in CODE from the CLAIM — `_content_tokens(claim_statement) - subject_tokens`, where `subject_tokens` is the
    code-derived retrieval-saturated topic set (model-independent). The model does NOT supply the property, so
    it cannot under-declare it (or absorb property words into a subject phrase) to collapse the floor (T2-D12).
    require-ALL: the quote must contain EVERY distinctive token — one fragment of a compound property does not
    earn credit. Subtracting `subject_tokens` is a RECALL aid (don't force the quote to restate the saturated
    subject). Witnesses on-property topicality, NOT entailment — a passage carrying all distinctive tokens in
    unrelated senses still passes; direction stays the model's label."""
    if not _quote_admissible(quote, source_text, min_tokens):
        return False
    distinctive = _content_tokens(claim_statement) - subject_tokens
    if not distinctive:                                    # claim fully saturated → nothing distinctive → uncertain
        return False
    return distinctive <= _content_tokens(quote)           # require ALL distinctive claim tokens (T2-D12)
