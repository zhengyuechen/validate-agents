...
- **BP-1 (overflow trigger — what counts as a divergence vs a domain error; resolved at plan time, the spec under-specified it)** The spec said "the integrator returns the overflow step" but did not define *when* a step overflows, and the existing safe evaluator collapses a magnitude blow-up (`inf`) and a domain error (`nan` from `log`/`sqrt` of a negative) into one `ValueError`. That matters: a domain error strikes at a **dt-stable time**, so it would *converge* under refinement and could **false-refute** — the very bug class B-D7 guards. Resolution: (a) `_eval_expr` gains `allow_nonfinite=False`; default is byte-identical (complex rejected, non-finite raises, Pow-overflow raises), and **only** the overflow-capturing integrator passes `True`, which relaxes *only* the finiteness raise (returns `inf`/`nan`) — the complex guard and every structure/whitelist guard stay unconditional, so the security surface is unchanged. (b) A divergence = the state reaching `inf` **or** crossing a module constant `_DIVERGENCE_MAG = 1e100`; `t_of` = time to cross it. A `nan` mid-run → **raises → uncertain**, never a divergence (the safe direction). `_DIVERGENCE_MAG` is **verdict-invariant** — it sets only the `t_of` scale, not whether `t_of` converges vs recedes — so it is an internal numerical constant, not a policy knob. **Known v1 limitation:** the magnitude/`inf`-vs-`nan` split is a heuristic; a genuine domain error at large magnitude could be miscounted as a divergence, but it is then still gated by the §3a convergence test and the §3b window check before it can refute.
