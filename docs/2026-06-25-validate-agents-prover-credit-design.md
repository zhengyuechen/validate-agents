...
- **Purely-definitional artifact (FG/PC-D6 interaction, open):** with the exemption at both gate sites, an artifact whose root claims are ALL definitional could reach `internally_validated` with zero real code-witnessed checks (degenerate — every premise, no substantive core). Unlikely in practice (a real idea has a non-definitional novel_core), but it is a cardinal-rule edge. Candidate guard (needs owner go, touches the gate): the strict-validation gate additionally requires ≥1 NON-definitional root claim with a real independent check. Surfaced for decision; not built. Sharpened by FG-1 (which lets unfalsifiable claims flow to the gate) — worth deciding alongside the falsifiability work.
