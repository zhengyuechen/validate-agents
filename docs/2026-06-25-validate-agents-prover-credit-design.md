...
- **PC-D6 gate authorization:** RESOLVED — owner-authorized; the exemption is implemented at BOTH gate sites (`AtomicClaim.status` and `_has_independent_external_check`, the strict-validation backstop — a final review caught that the first pass wired only one).

- **I2 — the designer prompt is limit-shaped, narrower than PC-1b's stated reach.** `design_computation` uses `COMPUTATION_DESIGNER`, written for limit-recovery ("reduces to the established result in the stated regime"). PC-1b reuses it for ALL mathematical claims, but a non-limit symbolic identity/equality doesn't fit that shape, so in practice only limit-shaped math claims earn executor credit today; others fall closed (uncertain → no credit → no say-so, safe but under-credited). This matches §4's honest ceiling ("steps that resist symbolic encoding earn no credit") but the *prompt* is the binding limit, not the executor. Follow-up: generalize `COMPUTATION_DESIGNER` to symbolic identities/equalities (carefully — it's shared with `inject_limit_checks`), or split a derivation-designer. Fails closed, so not a blocker.

- **Purely-definitional artifact (FG/PC-D6 interaction) — RESOLVED (guard shipped).** The PC-D6 exemption keys on the decomposer's (LLM) `type` label, so an all-definitional root set would satisfy the strict gate's witness requirement on zero real checks — re-importing say-so through the type label, and composing with FG-1 (which lets unfalsifiable claims reach the gate) into a hole neither opens alone. Closed by the strict-gate guard `any(c.type != "definitional" and self._has_independent_external_check(c) for c in rs)` — the artifact must rest on ≥1 non-definitional code-witnessed root. Owner-authorized; shipped with tests (premises-only → not validated; mixed → still validated). Also re-seals the falsifiability spec's §2 backstop.

- **`novel_core`-must-be-witnessed (defense-in-depth, deferred).** The shipped guard keys on `type` (every claim has one) and guarantees ≥1 real witness, but NOT that the *novel_core itself* is witnessed — a mislabeled-definitional `novel_core` could still ride through on a trivial witnessed sibling. The tighter guard (a `novel_core` claim may not be definitional-exempt) keys on `role` (LLM-set, sometimes None), so it is deliberately NOT coupled to the cardinal `type`-based guard. Logged as defense-in-depth, not an FG-1 blocker.
