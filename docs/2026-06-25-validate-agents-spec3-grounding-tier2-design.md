...
- **Code witnesses (new in Tier-2):**
  ...
  2. **On-property topicality (non-vacuous)** — the quote contains **ALL** the claim's **distinctive asserted-property** tokens (require-ALL, T2-D11), not merely the entity/topic tokens retrieval already saturated, and not just one fragment of a compound property. This is what makes "topicality" mean something (retrieval queries on `claim.statement`, so plain claim-overlap is vacuous — §5). *(Two disclosed costs: (a) co-saturation — if the whole property is topic-defining it self-subtracts → `prop_distinctive` empties → `uncertain` (fail-closed); (b) single-token residual — when the distinctive set is one ambiguous word, require-ALL = any, so it can be matched in an unrelated sense (entailment not witnessed). §5/§10.)*
...
ground_claim(claim, …):
  ...
  for c in citations:
     ...
     elif c.direction == "supports" and prop_ok and (prop_distinctive <= _content_tokens(c.quote)):
         passing.append(art)                                       # §5 on-property floor: quote contains ALL distinctive tokens (T2-D11)
...
**On-property floor (supports ONLY, §5):** the quote's content tokens contain **ALL** of `prop_distinctive` (`prop_distinctive <= _content_tokens(quote)` — the claim's distinctive asserted-property tokens, after subtracting the subject), gated by Guards 1+2 (`prop_ok`). `_support_quote_valid(...)` = `_quote_admissible(...)` **and** the require-ALL on-property check. **Require-ALL, not any-overlap (T2-D11):** a compound property like `temperature-independent` tokenizes to `{temperature, independent}`; crediting a quote that shares only one fragment lets an off-property quote earn credit (`temperature` variation of a *different* observable; `independent` in an unrelated sense). Requiring every distinctive token closes that; the residual is a single-token distinctive set, where ALL = any (irreducible without semantics — §5).
...
- **The floor (require-ALL — T2-D11, the soundness anchor):** `prop_distinctive <= _content_tokens(quote)` — the quote must contain **every** distinctive property token, not just one. `_content_tokens` splits on non-alphanumerics, so a compound property `temperature-independent` → `{temperature, independent}`; an *any-overlap* floor credited an off-property quote that shared only the high-frequency fragment (`temperature` variation of a *different* observable; `independent` in an unrelated sense — a measured false-credit, caught in review). Require-ALL closes it. **This makes require-ALL the soundness mechanism and demotes subject-subtraction to a recall aid:** even with no subtraction a subject-only/fragment-only quote fails (it lacks the full property); subtraction only spares a genuine quote from being forced to restate the subject formula.
...
**The other direction is fail-closed — and this is a DECISION, not a residual to patch (note):** if retrieval keys on the *property* (e.g. all hits are about superconductivity), the property co-saturates → `prop_distinctive` empties → Guard 2 → `uncertain`. That is **safe** (recall drops for topic-defining/well-corroborated-property claims; never a false-support). A "refine-never-empty" variant (make `subject_phrase` primary and *restore* saturation-removed tokens when they'd empty the property) was considered to recover that recall and **rejected**: it reopens a *rich-corpus* leak the union closes — a model emitting `asserted_property="<formula>"` + `subject_phrase=""` gets the saturated formula restored, and a formula-only quote earns credit. Saturation subtracts the formula in a rich corpus **regardless of what the model emits**; that model-independence is precisely why the union is fail-closed and any LLM-signal-gated restoration is not. The recall cost is the *price* of ungameable subtraction, disclosed in §2/§10 + the basis. A genuine fix needs a **non-saturation subject signal** (so subject and corroborated-property are separable) — deferred to the ≥2 slice (§1, T2-D9), where corroboration is the right lever for the well-corroborated case anyway.
...
**What it catches / doesn't (honest):** catches the measured off-property false-credits — a synthesis sentence sharing only the subject/formula, AND (require-ALL, T2-D11) a quote sharing only one fragment of a compound property (`temperature` of a different observable; `independent` in an unrelated sense). Does **not** catch: (a) a polarity flip that contains the *full* distinctive property phrase (claim "temperature-independent", quote "not temperature-independent") — passes the floor; direction stays loud (§6 + deferred ≥2); (b) a **single-token distinctive** property — when `prop_distinctive` is one ambiguous word (an inherently one-word property, or a compound co-saturated down to one fragment), require-ALL = any, so that word can be matched in an unrelated sense. Both are entailment-not-witnessed residuals, irreducible without semantics. The floor witnesses *on-property topicality*, never *entailment*.
...
- **Property floor** — Guard 1: an `asserted_property` with a token not in the claim → fail-closed (uncertain). Guard 2: `asserted_property` entirely subject/entity (property-as-subject) → `prop_distinctive` empty → uncertain. **Subject-subtraction-as-recall:** a genuine quote stating the full distinctive property but omitting the subject formula passes once the subject is subtracted, and false-rejects without subtraction (subtraction is a recall aid, not soundness). **Honest-boundary pins (no over-claim):** (a) a contradicting quote *mislabeled* `supports` that carries the full property phrase (a negation, "not temperature-independent") passes the floor — irreducible polarity residual, caught only when labeled `contradicts` (§6); (b) a **single-token** distinctive property matched in an unrelated sense passes — irreducible without semantics. Both carried by §6 + the deferred ≥2, never claimed "caught" by the floor.
