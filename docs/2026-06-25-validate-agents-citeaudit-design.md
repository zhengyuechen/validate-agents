...
- **arXiv:** reuse `web_search.ArxivBackend.search(query=name, max_results=arxiv_rows)` → `Article{title, summary, url, published}`.
- **Crossref:** new `_crossref_title_search(name, rows)` — `httpx` GET `https://api.crossref.org/works?query.bibliographic=<name>&rows=<rows>`, parse `message.items[*]` → `{title: item.title[0], authors, year, doi/url}`. Keyless (same Crossref host the `DoiResolver` already uses, different endpoint).
...
2. Merge resolved refs into the bibliography: `build_references` gains an optional `asserted_refs: list[Reference] | None = None` parameter; the resolved refs are folded into its `by_locator` dict (keyed by `normalize_id`, so a narrative ref that coincides with a retrieved/provided one merges rather than duplicates) **before** the existing numbering/key/sort tail — so they receive `[n]` + a BibTeX entry. Resolved refs have `cited_by=[]`, so the existing `(0 if cited_by else 1, locator)` sort places them after claim-cited refs. `render_report` then maps each resolved `name → ref.number` for the inline `[n]`.
3. `render_report(art, refs, audit_map)` and `to_bibtex(refs)` as before.
