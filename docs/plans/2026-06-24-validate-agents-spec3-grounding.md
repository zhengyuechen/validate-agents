...
**Interfaces:**
- Produces:
  - `fetch_source_text(locator: str) -> tuple[str, dict] | None` — the live network fetch: return `(text, {"locator","title","url","year"})` or `None`. **v1 substrate is the abstract / clean prose** (arXiv summary / crossref abstract / URL→text) — the *more reliable* choice, not a compromise: a table/figure number fails §6 regardless of PDF fetch, and full-text only adds body-prose numbers bought with PDF→text noise that breaks the verbatim-quote substring check. Full-text body prose is a **v1.x lever via clean HTML (arXiv/ar5iv), not raw PDF**. Network; injected in tests.
...
**Gaps / flags:** (a) **Abstract/clean-prose fetch (Task 6)** — v1 fetches abstracts, not full-text PDF (spec §3 rev5: the *more reliable* substrate — table/figure numbers fail §6 regardless of PDF, full-text adds only body-prose numbers at the cost of PDF→text noise that breaks the quote check; full-text body prose is a v1.x clean-HTML lever). Compound effect (stated in §3): with the say-so strip, v1 mostly **demotes** and rarely **promotes**; a number absent from the abstract → `unconfirmed` (nothing shown), not `inconclusive`. Fail-closed, net-stricter — correct, not additive. (b) The `§11` interior/boundary contradicts pins and the no-false-contradict-from-wrong-regime probe live in Task 5's tests; the wrong-conditions/G-D5c/sub-Kelvin probes in Task 4's; the emu-denominator in Tasks 2/5. The Task-5 and Task-4 reviewers carry the no-ground-truth load.
