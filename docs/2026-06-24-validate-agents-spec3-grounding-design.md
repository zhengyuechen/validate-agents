...
- **G-D9 (scale-table factors are adversarially verified or out-of-table; named tolerance; whole-token match)** A wrong factor is a code-owned, reproducible, confidently-wrong conversion — worse than LLM-converts. Every entry carries a both-directions reference test passing within **`reference_rel_tol` (1e-3)**; an unverified entry is out-of-table (`unconfirmed`). The energy row uses **full-precision** factors (exact, free) — at 4 sig figs `1 meV → 11.600 K`, a ~3.6e-4 miss that **passes** `1e-3` (~2.6× margin) but **fails** `1e-4`; full precision passes its own guard at either tolerance. `1e-3` is the chosen tolerance (`1e-4` leaves no room for legitimately-rounded future hand-added entries). The in-table lookup is a **whole-token** match (F2): a token with a denominator (`emu/g`, `emu/cm³`, `emu·mol⁻¹`, `emu/Oe`) is out-of-table — only **bare `emu`** = moment — so a per-mass value can't launder as an absolute moment. `emu·mol⁻¹` molar susceptibility (4π/per-mole/Oe) is deferred until its factor is reference-verified.
