...
- **Lens 2 — Magnitude / Detectability Executor** (numpy/scipy; predicted effect vs sensitivity/bounds; catches "real but numerically inert"). Same `ComputationPlan`/`ComputationResult`/`ComputationVerdict` shapes, `kind: magnitude`. Built *after* the symbolic lens is solid. **Design seed (owner):** the plan compares four quantities — `predicted_effect`, `baseline_or_null`, `sensitivity_or_bound`, `decision_threshold` — and the code verdict is `detectable` iff the predicted effect clears the sensitivity/bound by the threshold (below → `fail`: numerically inert; can't-compute → `uncertain`). **The danger is the model inventing a friendly sensitivity**, so — exactly as `expected_source` does in the symbolic slice — `sensitivity_source` and `bound_source` are mandatory and surfaced *loud* in the `CheckRecord.basis`; a thresholds-without-source plan is treated as `uncertain`, never `pass`. Discipline unchanged: structured plan → preregistered threshold → numpy execution → code verdict → `CheckRecord`.
