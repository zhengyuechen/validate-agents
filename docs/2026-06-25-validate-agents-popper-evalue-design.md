...
- **A new computed field `AtomicClaim.evidence_strength` (and an artifact-level rollup)** — `@computed_field @property` returning `_claim_evalue(self)`. This is the **only** new gate-adjacent output, and it is **read-only signal**: the arbiter (`arbiter.py`) and the report `basis` surface it ("evidence E=…, validates-at-≥10" alongside the count); **nothing branches the verdict on it**. `status`/`verdict_class` are unchanged.
...
**Prerequisite for the gate-migration (must precede gating on E):** a **concordance check** — assemble a small labeled gold set of known-valid / known-refuted ideas, run the pipeline, and confirm `evidence_strength` (E) *separates* them (high E on valid, low on refuted) before any verdict depends on it. This is the Co-Scientist "validate-the-validator" idea, and per the cross-cutting principle (§11/the review) it is the **gate for shipping the gate-migration**, not an afterthought. It also calibrates `q0`/`κ`/`α`.
...
- `valagents/agents/arbiter.py` + `valagents/cli.py` — surface `evidence_strength` in the arbiter record / report basis (read-only).
...
1. **Even a *surfaced* E can mislead.** A report basis showing "E=42, validates-at-≥10" invites a human to treat the claim as validated even though the verdict didn't gate on it. Does surfacing a heuristic-with-guarantee-shaped-numbers re-introduce false authority through the back door? Should the surfaced label avoid `1/α` framing entirely and present E only as a relative strength (e.g. a percentile vs the gold set), to not imply a bound?
